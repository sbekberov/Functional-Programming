...
abstract class PostSet extends postsetInterface {
   ...
   * and be implemented in the subclasses?
   */
  def union(that: PostSet): PostSet = that.filterAcc(_ => true, this)
   ...
   * and be implemented in the subclasses?
   */
  def mostLikedAcc(most: Post): Post
...
class Empty extends PostSet {
  ...
  override def mostLiked: Post = {
    throw new java.util.NoSuchElementException
  }

  override def mostLikedAcc(most: Post): Post = most

  override def descendingByLikes: PostList = Nil
...
class NonEmpty(elem: Post, left: PostSet, right: PostSet) extends PostSet {
  ...
  def filterAcc(p: Post => Boolean, acc: PostSet): PostSet = {
    val curr = if (p(elem)) acc.incl(elem) else acc
    right.filterAcc(p, left.filterAcc(p, curr))
  }
  override def mostLiked: Post = {
    right.mostLikedAcc(left.mostLikedAcc(elem))
  }

  override def mostLikedAcc(most: Post): Post = {
    if (elem.likes > most.likes)
      right.mostLikedAcc(left.mostLikedAcc(elem))
    else
      right.mostLikedAcc(left.mostLikedAcc(most))
  }

  override def descendingByLikes: PostList = {
    val most = mostLiked
    new Cons(most, remove(most).descendingByLikes)
  }
...
object GoogleVsApple {
  ...
  lazy val googleposts: PostSet = PostReader.allposts.filter(post => google.exists(post.text.contains))
  lazy val appleposts: PostSet = PostReader.allposts.filter(post => apple.exists(post.text.contains))
